...
function* getTechnologiesSelected(action) {
  try {
    if (action.payload.length > 0) {
      const response = yield axios.get(`/api/technologies/selected`, {
        params: { reqArray: action.payload },
      });
      let htmlTagList = '';
      ...
      yield put({
        type: 'SET_TECHNOLOGIES_HTML_TAG_LIST',
        payload: htmlTagList,
      });
    } else {
      yield put({ type: 'CLEAR_TECHNOLOGIES_HTML_TAG_LIST' });
    }
  } catch (error) {
